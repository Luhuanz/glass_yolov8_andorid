...
"""
下面这段代码定义了一个名为 AutoShape 的类，它是一个用于处理 YOLOv8 模型输入的类。
该类实现了模型的前向传播、数据预处理、后处理等方法。
在类的初始化方法中，首先根据输入的参数初始化了一些属性，如 NMS 的置信度阈值、IoU 阈值、是否为类别不可知等。
然后，根据输入的模型对象复制了一些属性，如模型的 YAML 配置、类别名称、步长等。
在 _apply() 方法中，将 to()、cpu()、cuda()、half() 等方法应用到模型张量上。
在 forward() 方法中，根据输入的参数执行了不同的操作，如将输入图像进行预处理、执行模型的前向传播、执行 NMS 后处理等。
最后，返回了一个 Detections 对象，其中包含了处理后的检测结果、输入图像、文件名等信息。

如果要使用 YOLOv8 模型进行目标检测，可以使用该类来处理输入数据，并获取检测结果。
例如，可以将输入图像转换为 PyTorch 张量，并将其传递给 forward() 方法，然后获取返回的 Detections 对象，
其中包含了检测结果的坐标、置信度、类别等信息。
"""
...
"""
下面这段代码定义了一个名为 Detections 的类，它是一个用于处理 YOLOv8 检测结果的类。
该类实现了模型的前向传播、数据增强、结果可视化等方法。
在类的初始化方法中，首先根据输入的参数初始化了一些属性，如输入图像、检测结果、类别名称、文件名等。
然后，根据检测结果计算了一些属性，如检测框的坐标、置信度、类别等。
在 _run() 方法中，根据输入的参数执行了不同的操作，如显示检测结果、保存检测结果、裁剪检测结果等。
在 show() 方法中，调用了 _run() 方法来显示检测结果。
在 save() 方法中，调用了 _run() 方法来保存检测结果。在 crop() 方法中，调用了 _run() 方法来裁剪检测结果。
在 render() 方法中，调用了 _run() 方法来渲染检测结果。
在 pandas() 方法中，将检测结果转换为 pandas DataFrame 格式。
在 tolist() 方法中，将检测结果转换为列表格式。
最后，在类中还定义了一些其他的方法，如 print() 方法用于打印检测结果，__len__() 方法用于获取检测结果的数量，
__str__() 方法用于返回检测结果的字符串表示等。
"""
class Detections:
